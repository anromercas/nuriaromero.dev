...
## Follow-up (2026-09-22): cobertura de similitud por n-gramas

Una pasada `review-reliability` sobre el commit `9b84ee5` detectó un hueco de cobertura real, no bloqueante, en la comparación nicho-vs-servicio: `normalizeQuestion()` solo hace minúsculas + quita `¿?¡!` + quita el literal `"en sevilla"` + colapsa espacios, y `isNearDuplicateQuestion()` exige igualdad exacta tras esa normalización. Cualquier variante de redacción fuera de esa normalización tan estrecha pasa desapercibida. Ejemplo construido por el revisor: la FAQ real `"¿Cuánto cuesta una tienda online en Sevilla?"` (`services.ts:672`) normaliza a `"cuánto cuesta una tienda online"`; una variante plausible de nicho `"¿Cuánto cuesta una tienda online completa?"` normaliza a `"cuánto cuesta una tienda online completa"` — no son iguales, así que pasaría sin ser detectada pese a ser un riesgo real de canibalización.

**Fix:** se añadió una capa de similitud por trigramas de palabras (Jaccard), reutilizando la misma lógica de `ngramSimilarity`/`wordNgrams`/`normalizedWords` de `scripts/check-seo-05.mjs`, encima del check de igualdad exacta existente (no lo sustituye). Umbral elegido: **0.35**, calibrado contra el corpus real (60 FAQs de servicio × 32-33 FAQs de nicho): la similitud real más alta encontrada fue 0.100 (`"¿Cuánto cuesta una página web para un restaurante?"` ~ `"¿Cuánto cuesta una tienda online en Sevilla?"`), y el par protegido restaurantes/diseño-web de este mismo task está en 0.071. El caso construido por el revisor puntúa 0.500. 0.35 deja margen amplio (3.5x) por encima de cualquier par real legítimo y bien por debajo de la señal de casi-duplicado.

RED/GREEN (usando el mismo caso construido, añadido temporalmente a `src/data/niches.ts` en la FAQ de comercios y revertido después de cada pasada):
- RED (checker sin la capa de similitud): `npm run build` + `node scripts/check-seo-07.mjs` → `SEO-07 intent check passed` (exit 0), no detecta el casi-duplicado.
- GREEN (checker con la capa de similitud): mismo build → `SEO-07 intent check failed (1 error) - niche FAQ is a near-duplicate (50.0% word-trigram similarity) of a service FAQ: "¿Cuánto cuesta una tienda online completa?" (/web-para-comercios-sevilla/) ~= "¿Cuánto cuesta una tienda online en Sevilla?" (/tienda-online-sevilla/)` (exit 1).

Tras revertir la reproducción temporal, `npm run check:seo-07` sobre el contenido real pasa limpio (32 niche FAQs vs. 60 service FAQs, 0 errores), incluyendo el par restaurantes/diseño-web y el resto de FAQs reales. Regresión completa repetida: `npm run build` + `check:seo-02` a `check:seo-14` (11 checks), todos en verde. `git diff --check` limpio; único cambio permanente: `scripts/check-seo-07.mjs`.
