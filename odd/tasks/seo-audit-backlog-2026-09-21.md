...
- [ ] SEO-11 Align local schema and brand — **Estado: parcial**; `ProfessionalService`/`Organization`, `Person`, `WebSite` y `Service` comparten entidad, naming público (`Nuria Romero`), área Sevilla + metropolitana y perfiles `sameAs` verificables. Se retiraron coordenadas no aprobadas y no se añadieron horarios, ratings, reviews, dirección pública ni GBP inventados. `npm run check:seo-11` pasa con 22 páginas y 10 servicios. Quedan pendientes decisiones humanas sobre naming GBP, horarios, geodatos/área definitiva y validación desplegada. Ver `docs/seo-audit/full-audit-2026-09-21/tasks/11-schema-local-y-marca.md`.
- [ ] SEO-12 Fix mobile UX and visual proof — **Estado: parcial**; hero mobile, consentimiento y botón flotante ajustados; `npm run check:seo-12` pasa con build local y guards sobre cuatro páginas de nicho. La validación visual real en 360/390/414/1440px, estados de cookies, foco/teclado y solapes queda pendiente por falta de navegador disponible. Ver `docs/seo-audit/full-audit-2026-09-21/tasks/12-ux-mobile-y-prueba-visual.md`.
- [ ] SEO-13 Build authority/backlinks plan — **Estado: parcial**; se publicó `/recursos/` como activo enlazable, se añadió un registro de outreach seguro con línea base y criterios de descarte, y `npm run check:seo-13` valida enlaces, política anti-manipulación y ausencia de métricas inventadas. La verificación externa de backlinks, dominios referentes, anchors, autoridad y toxicidad sigue pendiente por falta de una fuente autorizada. Ver `docs/seo-audit/full-audit-2026-09-21/tasks/13-autoridad-y-backlinks.md`.
...
- SEO-11 local evidence: `src/lib/schema.ts` now uses the public brand `Nuria Romero` consistently across the shared `ProfessionalService` + `Organization`, `Person`, `WebSite`, and `Service` graph; `areaServed` declares only Sevilla and its metropolitan area; `sameAs` remains limited to the two existing official profiles. Unverified coordinates, opening hours, ratings, reviews, street address, postal code, and GBP URLs are intentionally absent.
- SEO-11 focused verification: `npm run check:seo-11` completed `npm run build` and `scripts/check-seo-11.mjs`; Astro reported 22 pages, 0 errors, 0 warnings and the preexisting `Schema.astro` hint, then the checker passed all 22 rendered HTML files and 10 `Service` schemas. The checker also guards against unverified local proof and metadata brand drift.
- SEO-11 pending human/production evidence: confirm official GBP naming, real opening hours, approved geodata and final service area; then validate deployed JSON-LD with Schema Markup Validator/Rich Results Test and reconcile against legal/GBP data. No remote validation was run.


- SEO-12 local evidence: the home hero now uses a shorter mobile rhythm while preserving desktop spacing; the consent dialog retains all legal controls, adds `aria-modal`, bounded mobile height, internal scrolling, touch-sized controls, and visible focus; WhatsApp is smaller and inset on narrow screens. `npm run check:seo-12` runs the fresh build and checker, passing 22 generated pages plus contact CTA guards on the four niche routes.
- SEO-12 pending visual evidence: Playwright/CUA was unavailable in this worker environment, so no new screenshots or claims about real viewport visibility, cookie-state overlays, keyboard traversal, or no-overlap behavior are recorded. Existing untracked audit screenshots were not touched.

- SEO-13 local evidence: `/recursos/` is a public linkable hub for the existing author profile, portfolio and editorial resources; `docs/seo-audit/authority-backlink-register.md` records the 2026-09-21 baseline, owner, evidence requirements, rejection criteria, safe outreach template and link-quality checklist. `npm run check:seo-13` builds the site and validates the rendered hub, `llms.txt` discovery link, anti-manipulation language, and no fabricated backlink metrics. External backlink verification remains pending because no authorized data source is configured.
